...
def smearPDF(energy, E, PDF, res):
    ...
    PDF = PDF[np.searchsorted(OldE, E_values[:,:,1])]/np.sqrt(2*np.pi*(sig*E_values[:,:,1])**2)*np.exp(
...
def likelihoodPlot_2decays_DMs_better(decay_channels, true_params, radius, num_points, exposure, Sangle,
    ...
    if marginalize:
        ...
        fig, ax = plt.subplots()
        for m in np.arange(np.size(DMs)):
            ax.contourf(X, Y, np.transpose(sigmaLikelihoods[m]), [sigma5, 1.], colors = 'blue')
        for m in np.arange(np.size(DMs)):
            ax.contourf(X, Y, np.transpose(sigmaLikelihoods[m]), [sigma2, 1.], colors = 'yellow')
        '''
        for m in np.arange(np.size(DMs)):
            if DMs[m] == 1000.0:
...
interp1d(DMassPiPi, PiPiLikelihood, kind='linear',
    ...
    E2, PDF2 = getPDF(decay_channels[0], 1000.0)
    Ensemble1 = getEnsemble(decay_channels[0], 1000.0, 2.e-26, exposure, Sangle)
    Ensemble1 = smearEnsemble(Ensemble1, resolution)
    fig, ax = plt.subplots()
    ax.hist(Ensemble1, bins = 100, density = True)
    ax.plot(E1, PDF1/(np.sum(PDF1)*(E1[1]-E1[0])), 'k-')
    ax.plot(E2, PDF2/(np.sum(PDF2)*(E2[1]-E2[0])), 'b-')
    ax.set_xlim(0,800)
    fig.show()
    '''
